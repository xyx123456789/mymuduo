...
void TcpConnection::sendInLoop(const void *data, size_t len)
{
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
    {
        ...
        if (!fautlError && remaing > 0)
        {
            // 目前发送缓冲区剩余待发送的数据长度
            size_t oldlen = outputBuffer_.readableBytes();
            // 如果超过高水位标记，执行高水位回调
            if (oldlen + remaing >= highWaterMark_ && oldlen < highWaterMark_ && highWaterMarkCallback_)
            {
                loop_->queueInloop(std::bind(highWaterMarkCallback_, shared_from_this(), oldlen + remaing));
            }
            // 将剩余的数据写入缓冲区
            outputBuffer_.append((char *)data + nwrote, remaing);
            if (!channel_->isWriting())
            {
                // 一定要注册channel的写事件，否则不会给channel通知epollout
...
void TcpConnection::connectDestroyed()
{
    if(state_ == kConnected)
    {
        ...
        //把channel所有感兴趣事件，从poller中del掉
        channel_->disenableAll();
         //连接断开，执行回调
        connectionCallback_(shared_from_this());
    }
    //把channel从poller中del掉
    channel_->remove();
}
